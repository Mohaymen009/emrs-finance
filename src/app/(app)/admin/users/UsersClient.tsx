  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <form
          onSubmit={createUser}
          className="bg-white border border-slate-200 rounded-lg p-5 space-y-4 animate-fade-slide-in"
        >
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2.5">Divisions</th>
          ...
          <tbody>
            {initialUsers.map((u) => (
              <tr key={u.id} className="border-t border-slate-100 odd:bg-white even:bg-slate-50/50 hover:bg-slate-100/70 transition-colors">
                ...
                <td className="px-3 py-2.5 text-xs">{u.divisionCodes.join(", ") || "—"}</td>
                <td className="px-3 py-2.5">
                  <Badge color={u.isActive ? "green" : "slate"}>{u.isActive ? "Active" : "Deactivated"}</Badge>
                </td>
                <td className="px-3 py-2.5 text-xs">{u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleString() : "Never"}</td>
